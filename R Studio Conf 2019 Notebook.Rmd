...
Parsnip is tidy version of the popular caret package being developed by Max Kuhn (who also devleoped caret).This package creates a unified interface to models, organizes them by model type (e.g. Logitic regression, splines etc)and generalizes how to fit them.


First create a model specificaiton
...
In the cases of models like random forest where certain data dependant parameters like **mtry** need to be specified,
...
eu_ts3 <-  eu_ts2 %>%
            group_by(key) %>%
              mutate(avg_30day = slide_dbl(Price,~mean(.,na.rm = TRUE),.size=30))
...
```{r}
library(ggplot2)
ggplot(eu_ts2,aes(x=Date,y=Price)) + geom_line()+
...
A more realisitic appication for broom are cases where you want to bootstrap the parameter estimates of a model
...
Hypothetical outcome plots provide an alterntative way to visualize uncertainty by animating a finite set of individul draws rather than producing a static representation of distributions. 
...
The plot below uses bootstrapped samples to visualize uncertainity in the trend line. 
```{r ,message=FALSE, warning= FALSE}
attach(mtcars)
...
This is possibly the coolest package ever. According to the documentation:
...
1. Best practices for naming files: <https://speakerdeck.com/jennybc/how-to-name-files?slide=27>
2. mlflow for tracking ML experiments,sharing  and deploying : <https://www.mlflow.org/docs/latest/tutorial.html>
